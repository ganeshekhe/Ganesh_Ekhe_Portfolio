...
export default function Skills() {
  ...
  useEffect(() => {
    fetch(API_URL + "/skills")
      ...
      .catch((err) => console.log("Skill Load Error:", err));
  }, []);
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {skills.map((skill, index) => {
            // uploaded image icon?
            const isImage = skill.icon && skill.icon.includes("/");
            // lucide icon (text name)
            const LucideIcon = Icons[skill.icon] || Code2;
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="bg-indigo-100 p-3 rounded-xl flex items-center justify-center">
                    {isImage ? (
                      <img
                        src={`${API_URL}/uploads/${skill.icon}`}
                        alt={skill.name}
                        className="w-7 h-7 object-contain"
                      />
                    ) : (
                      <LucideIcon className="text-indigo-600 w-7 h-7" />
                    )}
                  </div>
