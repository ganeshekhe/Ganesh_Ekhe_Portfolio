...
export default function Hero() {
  ...
  /* -------------------------------------------
        FIXED PATHS FOR IMAGE / RESUME / CERTS
  ------------------------------------------- */

  const profileImg = profile.image
    ? `${API}/${profile.image}` // <= AUTO FIX
    : "https://cdn-icons-png.flaticon.com/512/149/149071.png";

  const resumePath = profile.resume ? `${API}/${profile.resume}` : null;

  const certificatePaths = Array.isArray(profile.certificates)
    ? profile.certificates.map((c) => `${API}/${c}`)
    : [];

  return (
    <section className="py-2 bg-gradient-to-b from-gray-50 to-white text-center">

      {/* Profile Image (auto fit any size) */}
      <motion.div
        initial={{ scale: 0, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        transition={{ duration: 0.6 }}
